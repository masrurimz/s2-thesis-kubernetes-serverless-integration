...
class IntelligentRoutingController:
    ...
    def __init__(self,
        ...
        logger.info("IntelligentRoutingController initialized",
    ...
    async def _control_loop(self) -> None:
        ...
        while self.is_running:
            try:
                ...
                logger.debug("Control loop cycle complete",
                           elapsed=elapsed, sleep_time=sleep_time)
    ...
    async def _make_routing_decision(self) -> None:
        ...
        try:
            ...
            if prediction and self.enable_intelligent_routing:
                target_weights = self._calculate_intelligent_weights(
                    current_stats, prediction
                )
                decision_type = "intelligent"
            else:
                target_weights = self.fallback_handler.get_fallback_weights(
                    current_stats
                )
            ...
            logger.debug("Routing decision complete",
                        type=decision_type,
    ...
    async def _get_prediction(self) -> Optional[Dict]:
        ...
        try:
            response = requests.post(
                f"{self.prediction_api_url}/predict",
                json={},
                timeout=5
            )
            response.raise_for_status()
            ...
            logger.debug("Prediction received",
    ...
    def _calculate_intelligent_weights(self, 
                                     current_stats: Dict, 
                                     prediction: Dict) -> Dict[str, int]:
        """
        ...
        try:
            ...
            if 'recommendation' in prediction:
                ...
                rec_knative = prediction['recommendation'].get('knative_weight', knative_weight)
            ...
            logger.debug("Intelligent weights calculated",
    ...
    async def _apply_weight_changes(self, target_weights: Dict[str, int]) -> bool:
        ...
        try:
            ...
            if (abs(target_weights["k3s"] - self.current_weights["k3s"]) < 5 and
                abs(target_weights["knative"] - self.current_weights["knative"]) < 5):
