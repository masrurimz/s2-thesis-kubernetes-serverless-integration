...
class PrometheusMetricsCollector:
    ...
    def get_decision_counts(self, scenario: str) -> Dict[str, int]:
        ...
        scale_out = self.query(
            'increase(routing_decisions_total{decision="SCALE_OUT"}[1h])'
        )
        if scale_out:
            counts["scale_out"] = int(scale_out)
            
        predictive = self.query(
            'increase(routing_decisions_total{decision="PREDICTIVE"}[1h])'
        )
        if predictive:
...
class RealTimeValidator:
    ...
    def run_load_test(self, scenario: str, duration: int) -> bool:
        ...
        workload_file = f"../infrastructure/load-tests/steady.js"
        ...
        try:
            ...
            result = subprocess.run(
                [
                    ...
                    "--summary-export", f"{self.results_dir}/k6_{scenario}_{int(time.time())}.json",
                    workload_file
                ],
                capture_output=True,
                text=True,
    ...
    def collect_metrics_during_run(
        self,
        scenario: str,
        run_id: int,
        duration: int
    ...
    def run_single_experiment(
        ...
        # Collect metrics
        metrics = self.collect_metrics_during_run(
            scenario, run_id, self.duration_sec
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Real-time hypothesis validation with Prometheus metrics"
    )
    parser.add_argument(
        "--duration",
        type=int,
        default=300,
        help="Duration per experiment run in seconds (default: 300)"
    )
    parser.add_argument(
        "--runs",
        type=int,
    ...
    parser.add_argument(
        "--live",
        action="store_true",
        help="Run live experiments (WARNING: takes 30+ minutes)"
    )
