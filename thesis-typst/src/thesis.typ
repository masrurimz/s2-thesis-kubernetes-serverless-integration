...
Results show S4 (Hybrid-predictive) achieves p99 latency = 118 ms with 104 SLO violations (n = 1 diagnostic), reducing p99 by 95.1% versus S1 (K8s-only) at 2,421 ms. The prediction-driven K8s scaling mechanism fires 52 proactive replica adjustments per run, reducing serverless traffic by 50% versus S3 (Hybrid-reactive) — 2,666 versus 5,354 serverless requests — yielding a monthly cost of USD 142 versus USD 147 for S3. S4 is the Pareto-optimal cost-performance point: no scenario achieves both lower cost and better latency. The GRU model achieves confidence 0.72--0.82 with inference latency 10--13 ms; trend extrapolation compensates for the model's 40--50% underprediction during load ramps. Statistical significance requires n = 5 replication (pending).
