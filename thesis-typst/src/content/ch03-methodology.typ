...
The extended 9-step horizon (135 s) covers the K3dAutoscaler's maximum provisioning delay (120 s) plus a 15-second safety margin. This follows the self-calibrating approach of ADAPT @adapt2026. The system validates the forecast horizon at runtime via the model-status endpoint. The simulated boot delay of 45 to 120 seconds matches measured cold-start times of public-cloud virtual machines at 55.9 s for AWS Linux and 124.1 s for GCP Linux, with warm starts between 22 s and 34 s @hao2021vmstartup; the GCP Linux cold start slightly exceeds the simulated maximum. The Kubernetes Cluster Autoscaler documentation reports 3 to 4 minutes for node provisioning on GCE, which is above this range, so the testbed models a fast boot. AutoScale measured setup times between 30 seconds and 1 minute for a locally created VM and 10 to 15 minutes for a cloud-provided one @gandhi2012autoscale. The delay is drawn once per pair and applied to both arms, so a paired difference excludes the boot time itself.
