...
LSTM (Long Short-Term Memory) is a recurrent neural network architecture. Hochreiter and Schmidhuber introduced it. It learns long-term dependencies in sequential data. The key innovation is the cell state. The cell state holds information and is regulated by three gates: the forget gate, the input gate, and the output gate. LSTM has been widely applied to cloud workload prediction @mondal2023toward. However, its three-gate architecture introduces large computational overhead. For a hidden state of size h, each LSTM cell requires $4h(h + x) + 4h$ parameters, where x is the input dimension. This overhead causes slower training and a higher parameter count.

GRU (Gated Recurrent Unit) is simpler than LSTM. Cho et al. proposed it @cho2014learning. GRU combines the forget and input gates into a single update gate. It also merges the cell state and hidden state. The reset gate controls how much past information to forget. The update gate controls the balance between the previous hidden state and the candidate activation. Chung et al. ran an empirical evaluation @chung2014empirical. They found that GRU achieves comparable or superior performance to LSTM on many sequence modeling tasks. The GRU cell needs about 25% fewer parameters than the LSTM cell, from three instead of four gate parameter sets. @mondal2023toward trained LSTM, BiLSTM, and GRU under one protocol (TensorFlow 2, Adam, 200 epochs, batch 512, MSE loss) on real Google clusterdata-2011-2 sequences, and tested on ten held-out sequences. LSTM and GRU used the same capacity of 50 hidden units. GRU was best on all three accuracy metrics: MSE 0.00194 against 0.00195, RMSE 0.04360 against 0.04367, and MAE 0.03057 against 0.03274. Training took 0.75 s against 1.44 s. Inference time was effectively identical, 0.0651 s against 0.0661 s. The paper does not state the hardware it measured on, so only the comparison within that paper is meaningful. Each model is also reported at its own best step size, 24 for GRU against 47 for LSTM. This research therefore measures its own latency constraint on the testbed (about 40 ms against a 50 ms target, Chapter 4) rather than borrowing theirs.
