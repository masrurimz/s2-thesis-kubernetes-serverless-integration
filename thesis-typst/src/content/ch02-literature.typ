...
Two primary patterns have been identified in the literature. The Overflow Model routes all traffic to Kubernetes until saturation is detected by CPU, memory, or latency metrics, at which point excess traffic overflows to the serverless backend. This pattern is reactive—it responds to saturation after it occurs, and the latency between detection and serverless capacity becoming available creates a window during which SLO violations may occur. The Predictive Routing pattern uses a forecasting model to anticipate traffic changes and proactively adjust traffic distribution before capacity limits are reached. This pattern is the basis for the architecture proposed in this research. AAPA @aapa2025 demonstrated that archetype-aware confidence weighting reduces SLO violations by 50%, motivating the forecast-confidence gating used in this thesis, while BACC @bacc2026 introduced budget-aware conformal autoscaling that adapts provisioning aggressiveness to remaining error budgets. HyPA @hypa2023 blends proactive forecasting with reactive scaling and automatically updates its prediction model when distributional shifts occur, recommending asymmetric scale-up/scale-down behavior, and STaleX @stalex2025 integrated spatiotemporal features with weighted PID controllers for microservices. These recent approaches demonstrate the growing interest in prediction-aware hybrid autoscaling @kapetanios2022.
...
This research extends ElaX in three significant ways. First, GRU substitution replaces LSTM with GRU as the workload predictor, reducing inference latency from approximately 100ms to approximately 40ms while maintaining comparable prediction accuracy. Second, routing-first decision logic extends the original resource allocation adjustment to route traffic between platforms—Kubernetes and serverless—adding a cross-platform dimension to the elasticity decision. Third, a priority-based action hierarchy (SCALE_OUT, PREDICTIVE, OPTIMIZE_COST, MAINTAIN) integrates both reactive SLO monitoring and proactive GRU predictions into a unified decision framework, ensuring that SLO recovery always takes precedence over cost optimization.
