...
Forecast-to-actuator alignment: The scaling model must be corrected so that effective-capacity forecasts produce differentiated replica targets (align alpha to effective capacity, not saturation). The provisioning delay should be measured live to set the forecast horizon, following the self-calibrating approach of ADAPT @adapt2026. A real 15-second ClarkNet-trained GRU artifact that passes the promotion gate must then be bound before a clean full-delivery paired rerun can test H2.
