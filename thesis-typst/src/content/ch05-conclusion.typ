...
This research designed, implemented, and evaluated a hybrid Kubernetes-serverless architecture with GRU-based workload prediction for intelligent traffic routing. The system addresses elastic scalability management in heterogeneous cloud environments through SLO-aware decision making. Three research questions were posed; each is answered below with direct reference to experimental evidence. The definitive paired S3/S4 comparison uses n = 5 counterbalanced pairs with variable ClarkNet load, utilization-based node consolidation, and a tuned reactive baseline (`2026-07-14_clarknet-tuned-paired-n5`). Earlier bundles are retained as historical records.
...
H2 (predictive scaling outperforms reactive): *Statistically supported (p = 0.030, d = -1.26, large effect).* The definitive paired experiment (n = 5 counterbalanced pairs, `2026-07-14_clarknet-tuned-paired-n5`) finds S4 mean p99 126.0 ms versus 188.5 ms for S3 (mean difference -62.5 ms, 95% CI [-100.9, -26.2], p = 0.030, Cohen's d = -1.26). S4 wins all 5 pairs and shows 80.2% fewer SLO violations (130 vs 656) at identical monthly cost (USD 163). The GRU forecast triggered 4--5 proactive routing decisions per S4 run. This result reverses the earlier non-significant finding (p = 0.38, d = -0.241) by incorporating three necessary conditions: an extended 9-step forecast horizon (135 s), utilization-based node consolidation matching Kubernetes Cluster Autoscaler semantics @k8sca-faq, and a tuned reactive baseline (120-second scale-down cooldown, 0.75 threshold) consistent with HPA stability guidance @serracanta2025hpa. Under variable ClarkNet load with all three-tier autoscaling active, the predictive controller maintains capacity during consolidation windows that the reactive controller releases too aggressively.
...
In summary, this research validates the hybrid mechanism and provides statistically significant evidence for the predictive advantage. The GRU predictor achieves target accuracy on synthetic data, the routing controller correctly shifts traffic based on SLO status, and hybrid routing reduces p99 latency by 95.1% over pure Kubernetes (H1, confirmed at n = 1). The definitive paired comparison (n = 5) finds S4 33.1% faster than S3 on p99 (126.0 ms vs 188.5 ms, p = 0.030, d = -1.26, large effect) with 80.2% fewer SLO violations at identical cost (H2, supported). The three-tier autoscaling architecture -- routing, pod scaling, and utilization-based node consolidation -- is demonstrated under realistic variable load. These findings confirm that the hybrid predictive architecture provides measurable latency and reliability advantages over both pure Kubernetes and reactive hybrid approaches, extending prior work on hybrid reactive/proactive controllers @hypa2023 and uncertainty-aware archetype autoscaling @aapa2025.
...
Adaptive forecast horizon: The 9-step GRU horizon deployed in the definitive experiment covers provisioning delays up to 120 s, which was sufficient for the ClarkNet workload. Future work should make the horizon fully adaptive, following the self-calibrating approach of ADAPT @adapt2026, dynamically increasing the required forecast steps based on a live EWMA of measured provisioning delays so that the horizon tracks real cluster conditions rather than relying on a fixed step count.
...
Extended statistical validation: H2 rests on the definitive n = 5 paired experiment with full treatment delivery; H1 remains a single n = 1 diagnostic. Future work should run n = 20+ replications to satisfy normality assumptions and provide confirmatory replication of the H2 primary result (p = 0.030) as well as an inferential test of H1, which is currently directionally supported at n = 1.
