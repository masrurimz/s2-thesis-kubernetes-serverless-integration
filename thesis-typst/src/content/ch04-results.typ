...
A representation search then asked whether any input or target representation beats that linear model. Twelve levers were specified against the per-fold autoregression under the leak-free fold protocol (`results/models/gru/2026-09-13_probes-representation/`). Eight are complete and four are queued. None beats the linear model. Selection-fold skill ranges from -0.01 percent for `robust_scale`, which is parity to the fourth decimal, to -3.68 percent for `nbeats`. The closest lever is `nlinear` at -0.30 percent, with a Diebold-Mariano p of 0.29. Parity holds within every day type, not only on aggregate: for `nlinear` against the autoregression, Friday evening reaches 46.0 against 45.6, Saturday 27.8 against 27.9, Sunday 25.2 against 25.3, and Monday 27.8 against 27.7, in RPS at the served amplitude. `nlinear` beats the autoregression on the test region and still fails the selection gate. The selection folds decide. The disclosed conclusion is that the deployed point predictor should be the linear autoregression. The network's parity is a limitation of this thesis, not a hidden fact.

Two further probes bound the deployment choice. First, coarser sampling lowers absolute error for any model. At 60-second buckets the fold error falls from 36.69 to 25.18 for the autoregression and from 36.69 to 25.30 for the network, in RPS at the served amplitude, because the target becomes coarser and the wall-clock horizon grows from 135 to 180 seconds. This is an interval and horizon change, not a modelling win. Second, a capacity-relative target detects capacity crossings where the point forecast does not (`results/models/gru/2026-09-13_t-cross-calibration/`). Crossing recall is 0.989 $plus.minus$ 0.009, against 0.008 to 0.042 for the autoregression plus threshold and 0.119 to 0.163 for persistence. The raw head is overconfident (per-step Brier 0.236 against the 0.049 linear reference), so an isotonic map was fitted per fold on that fold's inner-validation steps only; calibrated Brier falls to 0.043, under the reference. The decision rule for this target is recall at a bounded false-alarm rate: at the 0.80 recall target the calibrated operating point sits at threshold 0.45 with precision 0.543, false-alarm rate 0.210, and mean lead 3.9 steps, about 58 seconds. The labels exist only where the capacity ceiling is approached, which is the code default of six replicas rather than the paired configuration's ten.
...
#figure(
  ...
  table(
    ...
    [Mean p99 (ms)], [188.5], [126.0], [diff -62.5, p = 0.0312],
...
H2 is supported in this batch. S4 achieves 33.1% lower mean p99 latency (126.0 ms vs 188.5 ms), with a permutation p-value of 0.0312 (below alpha = 0.05) and a large effect size (Cohen's d = -1.26). The p-value reaches the design floor: 1/32 is the smallest p attainable with five pairs, because all five pairs fell the same way. The stored bundle artifact records 0.0304 from an older analysis routine, with 0.1216 for the corrected secondaries; the current `thesis experiment analyze` recomputes 1/32, and the secondaries return 1/32 as well, 0.1248 corrected. Both remain not significant. The deployed predictor `data/models/gru_model.pt` entered version control on 2026-09-12 (commit 98e17d1, LFS oid 7f8244bcc403c59a3e2be5d31b1ee5586f01c0052416c2705c0a325d1cc3099d) as the deployed artifact. The 2026-07-14 manifest records an empty predictor block, so this batch's weights cannot be verified, and later batches do not reproduce its significance. S4 wins all 5 pairs. The replication batches in @sec:replication bound this result: the effect keeps the S4-favorable direction, it does not reproduce at this significance, and the pooled verdict over those batches does not clear alpha. The 95% confidence interval [-100.9, -26.2] lies entirely below zero. S4 also shows 80.2% fewer SLO violations (130 vs 656) at identical monthly cost. The GRU forecast triggered 4--5 forecast-driven routing decisions per S4 run. Prediction therefore contributes to the latency advantage rather than consolidation noise. The progression from the initial non-significant result (p = 0.38, d = -0.241) to the definitive significant result (p = 0.0312, d = -1.26) shows that forecast horizon adequacy, node consolidation, and reactive-baseline tuning were the decisive factors.
...
The study validated several mechanisms. The GRU model meets its accuracy target on synthetic data. It achieves RMSE 5.2% of the mean load on the synthetic arm under the leak-free protocol, meeting the target, and 39.6% on the held-out region of the real deployment trace, where the target is not met and the model matches a linear autoregression on the same input window. Inference latency is about 40 ms. Real-traffic prediction is therefore a partial validation. The routing controller correctly implements the priority-based decision framework and exhibits all four action types. The confidence-gated forecast feeds Algorithm 2's replica planning. The corrected predictive mechanism fired 9 forecast-driven routing decisions in the S4 diagnostic run.
...
For the predictive-versus-reactive comparison (H2), the definitive paired experiment (n = 5 counterbalanced pairs, full treatment delivery) finds S4 mean p99 126.0 ms versus 188.5 ms for S3 (mean difference -62.5 ms, 95% CI [-100.9, -26.2] ms, p = 0.0312, d = -1.26, large effect). S4 wins all 5 pairs. This result reverses the initial non-significant finding (p = 0.38, d = -0.241) documented in the superseded section above. The reversal came from three changes: extending the GRU forecast horizon from 5 to 9 steps (135 s), correcting the scaling calibration from alpha = 1/r_saturation to alpha = 1/r_effective, and adding utilization-based node consolidation. S4 also shows 80.2% fewer SLO violations (130 vs 656) at identical monthly cost. The corrected secondary p-values are descriptive only.

Two caveats bound these conclusions. First, the n = 1 four-scenario diagnostic provides directional mechanism evidence but no inferential statistics for H1. Second, the definitive n = 5 paired comparison is clean and fully delivered, and its effect size is large (d = -1.26). Five pairs resolve only large effects: the paired standard deviation in that batch is 49 ms, so the design detects differences near 67 ms at 80 percent power, which places this batch at the edge of the design's resolution. Later batches show a paired standard deviation near 102 ms, where five pairs detect only differences above about 140 ms. Secondary metrics (p95, SLO) are not significant after multiplicity correction (corrected p = 0.12) and are reported as descriptive. The primary p99 test was pre-specified. Larger samples remain the confirmatory step for this hypothesis: the primary verdict rests on the pooled replication record, not on the single significant batch, so the next batch needs a declared forecast lead, a matched provisioning delay, and enough pairs to resolve the effect at the observed spread.

Taken together, the results directionally support H1 (the hybrid architecture outperforms the pure Kubernetes baseline at n = 1). For H2 they provide one significant batch rather than a robust effect: the primary p99 test in the definitive batch gives p = 0.0312 with d = -1.26, while the replication batches are null or marginal and their pooled verdict does not clear alpha. A batch can test H2 only when the predictive arm acts on its forecast. Runs that record no forecast-driven decision measure the reactive controller under an S4 label and cannot separate the two arms. The calibration repair showed that the original scaling model's double-counted safety margin caused S4's apparent underperformance, not the controller design. Recent work on uncertainty-aware autoscaling @aapa2025 and delay-derived planning horizons @adapt2026 provides design directions for further improvement. Benchmark studies @rayscale2026 show that even advanced autoscalers struggle to outperform well-calibrated baselines on cost.
