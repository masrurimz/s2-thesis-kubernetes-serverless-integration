...
This chapter presents the experimental results obtained from evaluating the hybrid Kubernetes-serverless architecture with GRU-based workload prediction. The results are organized into sections covering GRU model performance, system mechanism validation, the comparative evaluation across deployment scenarios, cost analysis, and an integrative discussion. Every quantitative claim in this chapter is traceable to the experiment registry under `results/`, as consolidated in `results/claims/FINAL_NUMBERS.md`. _Important caveat:_ the four-scenario comparative and cost results are drawn from the `2026-07-11_scaling_fix_n1` baseline, which is an *n = 1 diagnostic* run (H1); these are framed as diagnostic rather than confirmatory. All previous experiment bundles (February and July 8-10, 2026) were invalidated by six infrastructure fixes (Bugs 8-13: (1) enforced node CPU limits, (2) recalibrated `r_saturation_per_replica`, (3) a fairness cap on Kubernetes replicas, (4) calibration tooling, (5) a GRU-server hard gate, and (6) separation of prediction from routing). The predictive-versus-reactive comparison (H2), by contrast, is established by the definitive *n = 5* paired experiment (`2026-07-14_clarknet-tuned-paired-n5`), whose primary p99 result is statistically significant (p = 0.030).
...
The comparative evaluation compares four deployment scenarios: S1 (Kubernetes with HPA), S2 (serverless-only), S3 (hybrid with reactive routing), and S4 (hybrid with predictive routing). All scenarios replayed the ClarkNet HTTP trace at a mean of 73 requests per second (peak 164 RPS) under a calibrated node capacity of Docker `--cpus=1.0` per workload node (2.0 CPU total), with `max_k8s_replicas = 6` capped to schedulable capacity and no per-pod CPU limits. These are *n = 1 diagnostic* results from the `2026-07-11_scaling_fix_n1` baseline; the four-scenario comparison (H1) awaits replication, while the S3-vs-S4 paired comparison (H2) is reported below as a definitive n = 5 experiment. All four scenarios were run on this fixed infrastructure, so the comparison is like-for-like within a single bundle — unlike the superseded July bundles, in which S1/S2 and S3/S4 came from different controller configurations.
...
The four scenarios separate sharply into two regimes. S1 (pure Kubernetes with HPA) saturates under the ClarkNet load: its p99 reaches 2,421.3 ms and it accrues 6,717 SLO violations (7.66% SLO violation rate), far exceeding the 200 ms SLO threshold. The other three scenarios all keep the p99 well under the SLO — S2 (serverless-only) at 77.7 ms, S3 (hybrid-reactive) at 98.8 ms, and S4 (hybrid-predictive) at 118.2 ms — with SLO violation rates at or near zero (0.02%, 0.00%, and 0.12% respectively). The role of the hybrid scenarios is therefore assessed through two directed comparisons: the predictive scenario against the reactive scenario (S4 vs S3), and the hybrid predictive scenario against pure Kubernetes (S4 vs S1). Because only a single run is available per scenario for this table, no inferential statistics (t-tests, effect sizes, confidence intervals) are computed here; the S4-vs-S1 verdict is a descriptive *n = 1 diagnostic*, whereas the S4-vs-S3 comparison (H2) uses the definitive n = 5 paired design reported in the subsections below.
...
#emph[This section documents the initial clean n = 5 result, obtained with the 5-step (75 s) forecast horizon and an untuned reactive baseline. The definitive result -- obtained with the 9-step (135 s) horizon, a tuned reactive baseline, and utilization-based node consolidation -- is reported in @sec:definitive-h2 below.]
...
This repair reversed the effect direction. The earlier (confounded) paired comparison found S4 10 ms *slower* (mean diff +10.0 ms); the clean comparison with the corrected calibration finds S4 8.1 ms *faster* (mean diff -8.1 ms). The reversal demonstrates that the calibration mismatch was the root cause of S4's apparent underperformance, not the controller design itself. However, the effect remains small (d = -0.241) and not statistically significant in this initial experiment. The definitive experiment below demonstrates that this was due to insufficient forecast horizon and an untuned reactive baseline.

=== Definitive Paired H2 Result (n = 5, Tuned, Variable Load) <sec:definitive-h2>
...
H2 is *statistically supported*: S4 achieves 33.1% lower mean p99 latency (126.0 ms vs 188.5 ms), with a permutation p-value of 0.030 (below alpha = 0.05) and a large effect size (Cohen's d = -1.26). S4 wins all 5 pairs. The 95% confidence interval [-100.9, -26.2] lies entirely below zero. S4 also shows 80.2% fewer SLO violations (130 vs 656) at identical monthly cost. The GRU forecast triggered 4--5 predictive decisions per S4 run (`predictive_count` = 4--5), confirming that prediction contributes to the latency advantage rather than consolidation noise. The progression from the initial non-significant result (p = 0.38, d = -0.241) to the definitive significant result (p = 0.030, d = -1.26) demonstrates that forecast horizon adequacy, node consolidation, and reactive-baseline tuning were all necessary for an actionable comparison.

=== Independent Replication (2026-08-07) <sec:replication>

To verify repeatability, the paired experiment was re-triggered from the aligned codebase on 2026-08-07 with the same configuration as the definitive bundle (ClarkNet trace, counterbalanced paired design, seed 42, `max_k8s_replicas = 10` calibration override, 9-step horizon). The replication confirms the *direction* of H2: S4 won 4 of 5 pairs (S3 131.7 ms vs S4 109.9 ms, mean difference -21.8 ms; SLO violations 172 vs 101, -41%), with a consistent negative effect (d = -0.71). It does not reproduce the July significance (one-sided permutation p = 0.0958): the reactive baseline was faster in the replication runs (131.7 ms vs 188.5 ms), narrowing the gap. The replication is therefore reported as an independent, configuration-matched re-run that confirms the effect's direction and the experiment's repeatability, while the definitive July statistics remain the inferential claim. Reproducing the comparison with the *default* calibration cap (`max_k8s_replicas = 6`) produces different system behavior (S3 absorbs peaks via serverless overflow) and is documented separately in `thesis/DRIFT_ANALYSIS.md` §6b.
...
The cost proxy is interpreted as a model, not a billing observation. Break-even analyses distinguish workload rates at which serverless and VM-backed capacity change relative advantage @skyrise2025; billing and scheduling granularity can inflate measured serverless resource cost beyond raw consumption @demystifying2025; and keeping warm instances introduces overhead that affects hybrid cost comparisons @highcost2025. These anchors motivate reporting the proxy directionally and retaining the paired equal-cost result rather than claiming universal savings.
