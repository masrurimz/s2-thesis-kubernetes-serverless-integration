...
The study ran a counterbalanced paired experiment (n = 5 S3/S4 pairs, `2026-07-12_paired-h2-clean-v2` bundle) to test H2. All 5 pairs delivered complete GRU forecasts (56 eligible prediction cycles per run, 0 failures). The primary endpoint is the paired p99 latency difference. @tab:paired-h2-primary reports the result.
...
To verify repeatability, the study re-triggered the paired experiment twice from the aligned codebase on 2026-08-07. Those two initial batches (`2026-08-07_paired-h2_060200` and `2026-08-07_paired-h2_104918`, n = 5 each) were not retained. Their primary artifacts (result.json, k6 summary, paired_analysis.json) were deleted from the working tree before commit. Their `paired_analysis.json` files are `primary_unrecoverable` placeholders, and their numbers survive only as self-referential thesis text. The two batches are therefore excluded from the evidence record.

Two further paired batches ran on 2026-08-08 with fully retained artifacts. RUN1 (`2026-08-08_paired-h2_195150`, n = 5) was null (mean difference -4.0 ms, one-sided permutation p = 0.436). RUN2 (`2026-08-08_paired-h2_233726`, n = 5) was marginal (mean difference -58.0 ms, 95% CI [-161.5, -2.8], one-sided permutation p = 0.062). Both batches retained the S4-favorable direction. Pooled over the two 2026-08-08 batches (n = 10 pairs), the mean difference is -31.0 ms, one-sided permutation p = 0.188, d = -0.32. The full four-scenario replication (`2026-08-09_clarknet-replay_032257`, 4 scenarios, 5 runs each, 20 clean runs) tested the S4-vs-S3 comparison with the pre-specified Welch t-test and with Mann-Whitney U. The Welch test was not significant (p = 0.097). The Mann-Whitney test was significant (p = 0.032) with d = -1.30. The replication is batch-dependent. Some batches reach significance and others do not. The direction is consistently S4-favorable across every batch.
...
Cost analysis maps the local testbed to equivalent cloud services. It maps Kubernetes HPA pods to a managed control plane plus compute nodes, and Knative concurrency to provisioned function concurrency. It then projects a unified monthly cost from measured resource consumption. The projections below come from the `2026-07-11_scaling_fix_n1` baseline and are *n = 1 diagnostic* estimates.
...
S4 underperformed S3 in this diagnostic. S4's p99 was 1,393 ms versus 874 ms (+59%), and its SLO violations were 46,245 versus 23,913 (+93%). S4 failed the actuator-fidelity validity gate. The measured provisioning delay (70.5 s) plus the 15-second safety margin requires `ceil(85.5 / 15) = 6` forecast steps, but the deployed GRU model outputs only 5 steps (75-second forecast window). S4 delivered 55 predictions at 100% delivery rate, yet it recorded zero proactive scale-up actions (`predictive_count = 0`). The forecasts did not change the actuator behaviour. The 5-step horizon is adequate for the default 60-second provisioning delay estimate but not for the longer delays observed under high load. This finding validates the ADAPT-inspired design direction @adapt2026: the forecast horizon must adapt to the measured provisioning delay rather than remaining static.

=== Variable-Load Validation (ClarkNet, All Tiers Exercised)

The constant-load diagnostic confirms the mechanism but cannot test predictive scaling. Constant traffic gives the forecast nothing to anticipate. A ClarkNet variable-load experiment (`2026-07-13_clarknet-dynamic-node-n1`, n = 1) uses the real ClarkNet trace (30--164 RPS ramps, mean 73 RPS) with `max_k8s_replicas = 10` and the extended 9-step GRU horizon (135 s window). ClarkNet peaks naturally exceed the six-pod static envelope. They trigger dynamic nodes without artificial load inflation.
