...
- **Primary result (p99 latency, paired):** S3 mean 188.47 ms, S4 mean 126.00 ms, mean diff −62.47 ms, 95% CI [−100.87, −26.21] ms (entirely below zero), permutation p=0.0312 (pre-specified primary test, significant at α=0.05; the 1/32 design floor at n=5, because all five pairs fell the same way), paired Cohen's d=−1.2646 (large effect, S4 better). S4 won all 5 pairs. The stored `paired_analysis.json` records 0.0304 from an older analysis routine; the current `thesis experiment analyze` recomputes 1/32.
- **Secondary results (descriptive after multiplicity correction):** p95 latency: S3 mean 95.77 ms, S4 mean 81.89 ms, d=−3.5247, uncorrected p=0.0312, **corrected p=0.1248 (not significant)**. SLO violations: S3 mean 656.4, S4 mean 130.0, d=−1.3462, uncorrected p=0.0312, **corrected p=0.1248 (not significant)**. Throughput: diff ≈0, p=0.7147. Monthly cost: identical (USD 163/mo both scenarios).
...
- **Statistical interpretation:** The primary p99 test is pre-specified and significant at p=0.0312 (uncorrected; design floor). The artifact's multiplicity-corrected primary field is 1.0; corrected p-values for secondary metrics (p95, SLO) are 0.1248 (the stored artifact records 0.1216) and therefore **not significant**. H2 is supported for the primary p99 superiority claim only.
...
- **Status:** ✅ **SUPPORTED (primary p99, p=0.0312 uncorrected, d=−1.2646, large effect).** H2 latency superiority is established for the pre-specified primary p99 test. Predictor provenance: `data/models/gru_model.pt` entered version control on 2026-09-12 (commit 98e17d1, LFS oid 7f8244bcc403c59a3e2be5d31b1ee5586f01c0052416c2705c0a325d1cc3099d) as the deployed artifact; the 2026-07-14 manifest records an empty predictor block, so this batch's weights cannot be verified, and later batches do not reproduce its significance. Secondary metrics (p95, SLO) show large effect sizes in S4's favor but corrected p=0.1248 (the stored artifact records 0.1216) is not significant after the same four-fold correction; report them as descriptive only.
---
...
- **Data shape and the earlier probe set (2026-09-10):** the series autocorrelates 0.73 at lag 1, 0.62 at one minute, 0.54 at thirty minutes and 0.45 at one day, with a diurnal peak-to-trough ratio of 3.6, so a 135-second horizon is mostly local level and slope — linear structure. `results/models/gru/2026-09-10_predictor-probes/`: nine variants, every test-region `skill_vs_ols` negative — 120-sample window −0.0035 (different derived test start, so read the skill column rather than the RMSE), robust normalisation −0.0041, baseline −0.0067, log target −0.0192, calendar (time-of-day features) −0.378, pinball −0.713. Handing the network more structure did not help in that wave, and the variants closest to plain level-tracking did best — but the calendar row predates the canonical normalisation the 2026-09-13 wave uses, so its degradation cannot be read as a verdict on time-of-day information; it needs re-testing under the canonical contract.
- **Representation search (2026-09-13):** `results/models/gru/2026-09-13_probes-representation/`. Twelve representation levers vs the per-fold OLS autoregression under the leak-free fold protocol; 8 complete, 4 queued. None beats the linear model on the selection folds: skill from −0.01% (`robust_scale`, parity to the fourth decimal) to −3.68% (`nbeats`); closest `nlinear` at −0.30% (Diebold-Mariano p 0.29), which beats OLS on the test region and still fails the gate, so the selection folds decide. Disclosed conclusion: the deployed point predictor should be the linear autoregression; the network's parity is a limitation, not a hidden fact.
- **Interval and capacity-relative target (2026-09-13):** `results/models/gru/2026-09-13_probe-t-cross/`. 60-second buckets lower fold error for any model (36.69 to 25.18 served RPS for OLS, 36.69 to 25.30 for the network; horizon 135 to 180 s): an interval and horizon change, not a modelling win. The capacity-relative crossing target reaches recall 0.988 ± 0.009 against 0.008–0.042 for OLS plus threshold and 0.119–0.163 for persistence, at precision 0.336, false-alarm rate 0.598, and crossing-time MAE 3.50 steps (about 52 s). It fails the per-step Brier rule (0.233 vs 0.049, overconfident); decision rule is recall at a bounded false-alarm rate, calibration pending. Labels use the 6-replica code-default ceiling (199.8 served RPS), not the paired 10 (333.0). The crossing head targets the qualification rate that caps the point-forecast rule: in the pair batches every actionable cycle converted into a proactive scaleup, but only 3 to 5 of 57 eligible cycles cleared the threshold rule.
...
| Performance (H3) | Representation levers vs linear AR | `models/2026-09-13-probes-representation` | ⚠️ None of 8 complete levers beats the linear model on selection folds; deployed point predictor should be the linear autoregression |
| Performance (H3) | Capacity-relative crossing target | `models/2026-09-13-probe-t-cross` | ⚠️ Recall 0.988 vs 0.008–0.042 (OLS+threshold); fails Brier (0.233 vs 0.049); recall at bounded FAR rule, calibration pending |
...
| Performance (H2) | S4 vs S3 paired p99 | `experiments.2026-07-14-clarknet-tuned-paired-n5` | ✅ Supported (primary p99 p=0.0312, d=−1.2646, large; corrected secondary p=0.1248 not significant) |
...
2. **H2 (predictive vs reactive):** "Supported (primary p99, definitive n=5). S4 mean 126.00 ms vs S3 mean 188.47 ms, diff −62.47 ms, 95% CI [−100.87, −26.21] ms (entirely below zero), pre-specified primary permutation p=0.0312 (the 1/32 design floor at n=5), Cohen's d=−1.2646 (large). S4 won all 5 pairs." Report secondary metrics (p95, SLO) with the correction caveat (rule 6).
