...
def generate_k6_script(stages_path: Path, output_path: Path) -> None:
    ...
    the generated script is portable across machines. Stage names come from the
    trace the script opens at runtime (``stage_<NN>``, NN = bucket index), and
    the ramp aggregate unions the trace's ramp stages, computed in the script
    by the same rule the analysis side applies (``analysis.k6_stages``).
    """
 ...
 *
 * Per-stage latency: one Trend + Counter per trace stage (k6 stages do not
 * auto-tag samples), plus a ramp_* pair over the ramp stages — the window the
 * hybrid mechanism lives in and a whole-run p99 dilutes.
 *
 * Usage:
...
// One Trend + Counter per trace stage: k6 stages do not auto-tag samples, so
// the script emits one metric pair per stage (stage_<NN> = bucket index).
