...
inside ``resource_utilization.json``; no producer writes those keys, so any run with a
utilization file — which is every run — reported a zero-complexity workload through
that path. The demand series is the exported ``prom_rps`` query.
"""
...
def test_utilization_samples_do_not_produce_a_zero_dci(tmp_path):
    """The regression: a resource file used to shadow the demand series entirely."""
    (tmp_path / "resource_utilization.json").write_text(
        json.dumps([{"timestamp": 1.0, "pod": "p", "backend": "k8s", "cpu_millicores": 250.0, "memory_mib": 64.0}])
    )
