...
def analyze_statistics():
    ...
    print(
        f"   Difference: {np.mean(s4_p99) - np.mean(s1_p99):.1f}ms ({((np.mean(s4_p99) - np.mean(s1_p99)) / np.mean(s1_p99)) * 100:.1f}%)"
    )
...
""")
    ...
    output_path = (
        Path(__file__).parent.parent
        / "results/experiments/phase-b/2026-02-12_replicated-20runs/statistical_analysis.json"
    )
