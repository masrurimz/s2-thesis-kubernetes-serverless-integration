...
Evaluasi dilakukan melalui 30 eksperimen terkontrol yang direplikasi: 20 \emph{run} (4 skenario $\times$ 5 replikasi) dari eksperimen \texttt{fib33\_n5} dan 10 \emph{run} (2 skenario $\times$ 5 replikasi) dari eksperimen \texttt{fib33\_proactive}. Beban kerja menggunakan \emph{trace} ClarkNet yang direplikasi melalui k6 (40 tahap $\times$ 30 detik = 20 menit, rentang RPS 22--164, rata-rata 73). Catatan: hasil S1 dan S2 dilaporkan dari \texttt{fib33\_n5}, sedangkan hasil S3 dan S4 utama dilaporkan dari \texttt{fib33\_proactive} yang menggunakan mekanisme \emph{proactive routing} aktif. Eksperimen \texttt{fib33\_n5} untuk S3/S4 dijalankan sebelum implementasi \emph{proactive routing} sehingga tidak digunakan sebagai basis perbandingan H2.
...
\subsection{Optimasi Parameter Pengontrol}
\label{sec:controller-hpo}

Empat parameter pengontrol V3 dioptimalkan melalui \emph{screening} Optuna TPE (10 \emph{trial}, masing-masing 20 menit): \emph{target\_cpu\_util}, \emph{kp\_burn}, \emph{proactive\_trend\_threshold}, dan \emph{proactive\_approach\_ratio}. Tabel~\ref{tab:controller-hpo} menyajikan hasil \emph{screening}.

\begin{table}[htbp]
    \centering
    \caption{Hasil \emph{screening} HPO pengontrol (10 \emph{trial}, 5 teratas)}
    \label{tab:controller-hpo}
    \begin{tabular}{rrrrrr}
        \toprule
        Trial & \emph{target\_util} & \emph{kp\_burn} & SLO & p99 (ms) & Objektif \\
        \midrule
        \textbf{0} & \textbf{0,659} & \textbf{1,49} & \textbf{232} & \textbf{156} & \textbf{232} \\
...
Konfigurasi terbaik (Trial 0) menghasilkan 232 pelanggaran SLO dalam \emph{single-run} — penurunan 67\% dari 702 pelanggaran dengan parameter bawaan. Namun, validasi \emph{holdout} $n=5$ mengungkap fenomena \emph{overfitting}: rata-rata SLO melonjak ke 2.886 pelanggaran (vs 702 dengan default), dengan p99 rata-rata 311\,ms (vs 188\,ms). Tabel~\ref{tab:holdout-overfit} membandingkan hasil \emph{single-run} HPO dengan validasi \emph{holdout}.

\begin{table}[htbp]
    \centering
    \caption{Overfitting HPO pengontrol: \emph{single-run} vs validasi \emph{holdout} ($n=5$)}
    \label{tab:holdout-overfit}
    \begin{tabular}{lrr}
        \toprule
        Metrik & Parameter Default & Parameter HPO \\
        \midrule
        \emph{Single-run} SLO & 702 & 232 \\
        \emph{Holdout} mean SLO ($n=5$) & 702 & 2.886 \\
        \emph{Holdout} mean p99 (ms) & 188 & 311 \\
        \bottomrule
    \end{tabular}
\end{table}

\textbf{Penyebab}: \emph{target\_cpu\_util} = 0,659 (vs 0,5 default) mempersempit \emph{safety margin} K8s dari 100 RPS menjadi 131 RPS kapasitas model. Pada \emph{single-run} dengan kondisi menguntungkan, ini mengurangi perluasan ke \emph{serverless}. Namun under variansi sistem tinggi, margin yang lebih sempit menyebabkan saturasi K8s yang lebih sering dan parah.

Fenomena \emph{overfitting} pada HPO telah dipelajari secara ekstensif dalam konteks pembelajaran mesin \cite{cawley2010overfitting}, namun aplikasinya pada sistem \emph{online} dengan evaluasi yang mahal dan bising belum banyak dieksplorasi. Berbeda dengan HPO pada model ML (di mana evaluasi memerlukan hitungan detik), setiap evaluasi pada sistem kami memerlukan eksperimen 20 menit penuh pada \emph{shared resources}, dengan variansi dari penjadwalan CPU, waktu inisialisasi kontainer, dan kondisi \emph{runtime} lainnya. Tantangan ini beririsan dengan HPO non-stasioner \cite{nonstationary2022}, di mana performa relatif konfigurasi dapat berubah seiring waktu.

\textbf{Metodologi dua tahap}: Berdasarkan temuan ini, kami mengusulkan pendekatan dua tahap untuk HPO pengontrol sistem \emph{online}: (1) \emph{Screening} — $n$ \emph{trial single-run} untuk mengidentifikasi region parameter yang menjanjikan; (2) \emph{Confirmation} — $k$ kandidat teratas divalidasi dengan $m$ \emph{run} masing-masing, dengan seleksi berdasarkan \emph{median} (bukan \emph{mean}) untuk resistensi terhadap \emph{outlier}. Implementasi fungsi \texttt{run\_controller\_hpo\_robust()} mengotomatisasi pipeline ini. Parameter bawaan yang konservatif (\emph{target\_cpu\_util} = 0,5) dipertahankan ketika tidak ada kandidat yang menunjukkan peningkatan \emph{robust} dalam tahap konfirmasi.

\textbf{Surrogate-assisted screening}: Untuk mengatasi biaya evaluasi yang mahal, kami membangun \emph{Gaussian Process} (GP) \emph{surrogate} dari 20 titik data eksperimen yang ada (12 kombinasi parameter unik). \emph{Monte Carlo screening} terhadap 10.000 sampel menunjukkan hanya 5,5\% memenuhi konstrain p99 < 200\,ms. Kandidat teratas GP (\emph{target\_cpu\_util} $\approx$ 0,45, \emph{kp\_burn} $\approx$ 0,6) menunjukkan hasil menjanjikan pada $n=1$ (SLO=565, p99=156\,ms), namun konfirmasi $n=5$ mengungkap ini sebagai \emph{outlier}: SLO=5.803 dan p99=434\,ms. Tabel~\ref{tab:gp-holdout} membandingkan ketiga tahap.

\begin{table}[htbp]
    \centering
    \caption{GP candidate: $n=1$ vs konfirmasi $n=5$}
    \label{tab:gp-holdout}
    \begin{tabular}{lrr}
        \toprule
        Evaluasi & SLO/\emph{run} & p99 (ms) \\
        \midrule
        Default ($n=5$) & 702 & 188 \\
        GP $n=1$ (\emph{preliminary}) & 565 & 156 \\
        GP $n=5$ (\emph{holdout}) & 5.803 & 434 \\
        \bottomrule
    \end{tabular}
\end{table}

Temuan ini memperkuat pelajaran kunci: pada sistem \emph{shared-resource} dengan variansi tinggi, \textbf{tidak ada} deviasi parameter dari kalibrasi bawaan yang bertahan pada $n=5$. Parameter bawaan (\emph{target\_cpu\_util} = 0,5) yang dikalibrasi dari uji kapasitas tetap optimal. Pendekatan \emph{surrogate} mengadaptasi BOHB \citep{falkner2018bohb} dan KAPETÁNIOS \citep{kapetanios2022}.
