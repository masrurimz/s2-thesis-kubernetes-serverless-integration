...
\begin{enumerate}
    \item Bagaimana merancang prediksi beban kerja HTTP menggunakan \emph{Gated Recurrent Unit} (GRU) untuk aplikasi \emph{cloud}?
    \item Bagaimana merancang pengambilan keputusan dengan memodifikasi kerangka kerja ElaX untuk skalabilitas dan distribusi lalu lintas pada kluster heterogen (\emph{Kubernetes} dan \emph{serverless})?
    \item Bagaimana mengevaluasi mekanisme ElaX yang dimodifikasi untuk \emph{automatic scaling} dan \emph{load distribution} pada integrasi \emph{Kubernetes} dan \emph{serverless}?
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{H1 (Efisiensi biaya \emph{hybrid})}: Arsitektur \emph{hybrid} dengan distribusi lalu lintas cerdas memberikan efisiensi biaya yang lebih baik dibandingkan pendekatan murni (\emph{Kubernetes-only} atau \emph{serverless-only}) pada tingkat kepatuhan SLO yang setara.
    \item \textbf{H2 (Prediktif $>$ Reaktif)}: Mekanisme \emph{proactive routing} berbasis prediksi GRU menghasilkan lebih sedikit pelanggaran SLO dibandingkan mekanisme \emph{reactive routing} murni.
    \item \textbf{H3 (Adekuasi GRU)}: Model GRU memberikan prediksi yang memadai (kepercayaan $\geq$ 0,5, latensi $<$ 50\,ms) untuk mendukung keputusan \emph{routing} proaktif pada beban kerja HTTP.
\end{enumerate}
...
\begin{enumerate}
    \item \emph{Workload}: fib(33) \emph{CPU-bound} + 50\,ms I/O \emph{wait}, dengan \emph{trace} HTTP ClarkNet yang direplikasi melalui k6.
    \item Infrastruktur: kluster k3d pada satu mesin 16-inti AMD, HAProxy untuk \emph{routing}, Knative Serving dengan KPA.
    \item Metrik: latensi p99 (ambang SLO 200\,ms), pelanggaran SLO, proyeksi biaya bulanan (AWS EKS+EC2+Lambda).
    \item Tidak ada batasan CPU pada level pod; batas CPU pada level node Docker (\texttt{-{}-cpus}) adalah batas yang sebenarnya.
    \item Basis metode dan algoritma yang digunakan adalah ElaX dengan modifikasi pengontrol routing V3 berbasis kapasitas dan mekanisme \emph{proactive routing}.
\end{enumerate}
