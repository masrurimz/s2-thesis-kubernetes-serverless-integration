...
\begin{center}
    ...
    Hasil menunjukkan S4 (\emph{Hybrid-predictive}) mencapai latensi p99 = 188\,ms dengan 702 pelanggaran SLO per \emph{run}, 75\% lebih sedikit dibandingkan S3 (\emph{Hybrid-reactive}) dengan 2.805 pelanggaran (\emph{Cohen's d} = 1,21, \emph{large effect}). S4 juga 20\% lebih murah daripada S1 (\$149/bulan vs \$187/bulan) dengan tingkat kepatuhan SLO 98,4\%. Model GRU dioptimalkan melalui HPO Optuna TPE (30 \emph{trial}, RMSE 6,01\% $\rightarrow$ 4,75\%) dengan 400/400 prediksi sukses dan latensi inferensi 10--13\,ms. Temuan metodologis: optimasi parameter pengontrol \emph{online} melalui HPO \emph{single-run} menyebabkan \emph{overfitting} yang hanya terdeteksi melalui validasi $n=5$, menunjukkan perlunya kalibrasi berbasis uji kapasitas dan konfirmasi multi-\emph{run}.
