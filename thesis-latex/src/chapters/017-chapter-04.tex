...
\textbf{Penyebab}: \emph{target\_cpu\_util} = 0,659 (vs 0,5 default) mempersempit \emph{safety margin} K8s dari 100 RPS menjadi 131 RPS kapasitas model. Pada \emph{single-run} dengan kondisi menguntungkan, ini mengurangi perluasan ke \emph{serverless}. Namun under variansi sistem tinggi, margin yang lebih sempit menyebabkan saturasi K8s yang lebih sering dan parah. Temuan ini menunjukkan bahwa HPO pada sistem \emph{online} dengan variansi tinggi memerlukan validasi multi-\emph{run}, bukan optimasi \emph{single-run}. Parameter bawaan yang konservatif (\emph{target\_cpu\_util} = 0,5) dipertahankan sebagai konfigurasi produksi.
