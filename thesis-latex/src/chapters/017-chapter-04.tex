...
Model \emph{Gated Recurrent Unit} (GRU) dirancang dengan dua lapis rekuren (64 dan 32 unit) dan regularisasi \emph{dropout} 0,2 untuk memprediksi tren beban kerja HTTP jangka pendek. Model menerima \emph{sliding window} observasi \emph{request-per-second} (RPS) terkini dan menghasilkan nilai RUS yang diprediksi beserta skor kepercayaan (\emph{confidence}).

Selama eksperimen live, model GRU berjalan pada server FastAPI (port 8090) menggunakan inferensi CPU (\texttt{CUDA\_VISIBLE\_DAYS=''}). Hal ini disebabkan oleh ketidakcocokan GPU AMD Radeon 780M (gfx1103) dengan ROCm/HIP yang menyebabkan \emph{error} ``\emph{invalid device function}'' saat inferensi pada GPU. Inferensi CPU mencapai latensi 10--13\,ms per prediksi, yang memadai untuk interval keputusan 15 detik.
