...
\textbf{Surrogate-assisted screening}: Untuk mengatasi biaya evaluasi yang mahal, kami membangun \emph{Gaussian Process} (GP) \emph{surrogate} dari 20 titik data eksperimen yang ada (12 kombinasi parameter unik). GP ini memprediksi SLO dan p99 untuk kombinasi parameter baru dalam hitungan detik. \emph{Monte Carlo screening} terhadap 10.000 sampel parameter menunjukkan bahwa hanya 5,5\% memenuhi konstrain p99 < 200\,ms. Kandidat teratas GP (\emph{target\_cpu\_util} $\approx$ 0,45, \emph{kp\_burn} $\approx$ 0,6) divalidasi pada sistem nyata (\emph{n}=1 \emph{preliminary}): SLO = 565 (vs 702 default, $-20\%$), p99 = 156\,ms (vs 188\,ms, $-17\%$). Hasil ini menunjukkan GP dapat mengidentifikasi kandidat dengan deviasi moderat dari bawaan, namun memerlukan konfirmasi $n \geq 5$ sebelum adopsi. Pendekatan ini mengadaptasi BOHB \citep{falkner2018bohb} dan KAPETÁNIOS \citep{kapetanios2022}.
