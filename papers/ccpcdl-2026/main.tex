...
\begin{tikzpicture}[
    node distance=0.5cm,
    gate/.style={draw, rounded corners, minimum width=1.4cm, minimum height=0.6cm, align=center, font=\footnotesize},
    layer/.style={draw, rounded corners, minimum width=1.6cm, minimum height=0.6cm, align=center, font=\footnotesize, fill=blue!10},
    arr/.style={->, >=stealth, font=\footnotesize},
]
% Input
\node (x) {$x_t$};
% First GRU layer
\node[gate, right=0.8cm of x, fill=blue!15] (gru1) {GRU Layer 1\\(128 units)};
% Dropout
\node[gate, right=of gru1, fill=gray!15] (drop1) {Dropout\\(0.2)};
% Second GRU layer
\node[gate, right=of drop1, fill=blue!15] (gru2) {GRU Layer 2\\(128 units)};
% Dropout
\node[gate, right=of gru2, fill=gray!15] (drop2) {Dropout\\(0.2)};
% FC layer
\node[layer, right=of drop2] (fc) {FC\\(64$\to$1)};
% Output
\node[right=0.6cm of fc] (y) {$\hat{y}_{t+30}$};
% Arrows
\draw[arr] (x) -- (gru1);
\draw[arr] (gru1) -- (drop1);
\draw[arr] (drop1) -- (gru2);
...
\draw[arr] (fc) -- (y);
% Window annotation
\node[below=0.1cm of gru1, font=\scriptsize, text=gray] {60-step input window};
\node[below=0.1cm of fc, font=\scriptsize, text=gray] {30s horizon};
\end{tikzpicture}
\caption{GRU model architecture: two-layer GRU (128 hidden units each) with dropout and a fully connected output layer. Input: 60 time steps of RPS; output: predicted RPS 30 seconds ahead.}
\label{fig:gru_arch}
\end{figure}
...
\subsection{Impact on Cold-Start Variance}
...
For the reactive scenario (S3), 58.2\% of total tail-latency variance ($\sigma^2 = 88{,}171$) was attributable to cold-start and routing overhead. For the predictive scenario (S4), this dropped to 23.0\% ($\sigma^2 = 47{,}870$)---a 35 percentage-point reduction. Despite similar SCALE\_OUT counts (17--18 per run), the prediction pipeline produces substantially less excess variance, suggesting that confidence-modulated weight adjustments create more stable routing behavior even without explicitly triggering the PREDICTIVE action.

The cold-start penalty itself was measured at 682--1{,}235\,ms (mean 959\,ms) for Knative pod initialization, compared to 133\,ms warm-state p99. This represents an 826\,ms overhead above warm-state baseline. In production deployments, setting \texttt{minScale=1} on Knative would eliminate this penalty entirely, though at the cost of eliminating scale-to-zero savings during idle periods.
...
\begin{tikzpicture}[
    ...
    backend/.style={draw, rounded corners, minimum width=1.8cm, minimum height=0.7cm, align=center, font=\footnotesize, fill=gray!10},
    arr/.style={->, >=stealth, thick},
]
\node[box, fill=blue!15] (gru) {GRU Predictor};
\node[box, fill=orange!15, left=0.2cm of gru] (slo) {SLO Monitor};
\node[box, fill=green!15, below=of gru] (ctrl) {Routing Controller};
\node[box, below=of ctrl] (hap) {HAProxy};
\node[backend, below left=0.3cm and 0.05cm of hap] (k8s) {K3s\\(warm)};
\node[backend, below right=0.3cm and 0.05cm of hap] (kn) {Knative\\(scale-to-zero)};
\draw[arr] (gru) -- node[right,font=\tiny]{pred} (ctrl);
\draw[arr] (slo) -- (ctrl);
\draw[arr] (ctrl) -- node[right,font=\tiny]{weights} (hap);
\draw[arr] (hap) -- (k8s);
\draw[arr] (hap) -- (kn);
\draw[->,dashed,gray] (hap.west) to[bend left=40] node[left,font=\tiny,text=gray]{metrics} (slo.west);
\end{tikzpicture}
\caption{Hybrid system architecture. The GRU feeds 30-second predictions; the routing controller adjusts HAProxy weights between K3s and Knative based on SLO monitoring and predictions.}
\label{fig:architecture}
\end{figure}

\textbf{Prediction Layer}: The GRU server receives real-time traffic metrics (request rate, latency) and outputs a 30-second-ahead forecast with a confidence score in the range [0, 1]. The confidence is derived from the variance of recent predictions---low variance indicates high confidence.

\textbf{Control Layer}: The routing controller implements a priority-based action hierarchy (Algorithm~1) evaluated every 15 seconds. Four actions are ordered by priority:
\begin{enumerate}
    \item \textbf{SCALE\_OUT} ($p99 > 200$\,ms): Shift traffic to serverless in 10-point increments ($100/0 \rightarrow 90/10 \rightarrow \ldots \rightarrow 50/50$).
    \item \textbf{PREDICTIVE} ($p99 < 140$\,ms \& predicted $\geq$30\% increase): Maintain serverless engagement proactively.
    \item \textbf{OPTIMIZE\_COST} (stable \& underutilized): Reclaim serverless capacity.
    \item \textbf{MAINTAIN} (default): No action.
\end{enumerate}

\textbf{Execution Layer}: HAProxy distributes HTTP traffic between K3s (always-warm pods) and Knative (scale-to-zero, burst handling) according to dynamic weights adjusted via the HAProxy Runtime API.

\subsection{Routing Controller Algorithm}
...
\small
\textbf{Algorithm 1:} Routing Controller Decision\\[2pt]
\textbf{Input:} $p99$, GRU prediction $\hat{y}$, confidence $c$, current weights $w$\\[2pt]
\textbf{repeat} every 15s:\\
\quad \textbf{if} $p99 > 200$ms \textbf{then} SCALE\_OUT: shift $w$ toward serverless\\
\quad \textbf{else if} $p99 < 140$ms \textbf{and} $\hat{y}$ predicts $\geq$30\% $\uparrow$ \textbf{and} $c > 0.5$\\
\quad\quad \textbf{then} PREDICTIVE: maintain serverless weights\\
\quad \textbf{else if} stable \textbf{and} underutilized \textbf{then} OPTIMIZE\_COST\\
\quad \textbf{else} MAINTAIN\\
\textbf{until} forever
\end{minipage}}
\caption{Routing controller decision logic.}
\label{fig:algorithm}
\end{figure}

In a ramp-load validation experiment, the PREDICTIVE action triggered at $p99 = 146$\,ms (below SLO threshold) when the GRU predicted a 47\% load increase with 72\% confidence---demonstrating proactive serverless engagement before any SLO violation occurred. The system correctly shifted traffic weights from $100/0$ (pure K8s) through five SCALE\_OUT steps to $50/50$ (maximum serverless engagement) as load increased, then triggered PREDICTIVE to maintain readiness during the predicted surge.
