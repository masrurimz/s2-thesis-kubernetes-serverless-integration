...
4. **Cost analysis** confirms that execution-time signal choice materially affects ranking. In the 2026-02-21 all-scenarios rerun-v2 (`n=1` each) using serverless-specific app-duration sizing, measured costs are S1 $0.061, S2 $0.169, S3 $0.486, S4 $0.432 per 1200s. This run is directional and pipeline-validating (not inferential final), but it demonstrates that hybrid/serverless estimates must use serverless-specific duration signals rather than blended scenario averages.
