...
2. **Develop a modified ElaX algorithm** that integrates workload prediction with SLO-aware routing decisions in a hybrid Kubernetes-serverless architecture. Specifically:
   ...
   - Algorithm 2 (Cluster Controller): integrated into the routing daemon for real-time Kubernetes replica scaling using the resource model $R = \alpha \cdot x + \beta$, operating in reactive mode (S3, using observed load) or predictive mode (S4, using GRU forecast).
