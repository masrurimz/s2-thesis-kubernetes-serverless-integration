...
| `2026-08-07_paired-h2_060200` | definitive override (cap 10) — 5 pairs | S4 wins 4/5 pairs: S3 131.7 vs S4 109.9 (Δ −21.8, CI [−45.5, +2.0], d = −0.71, one-sided p = 0.0958); SLO violations 172 → 101 (−41%) |

**Interpretation (honest):** the replication *confirms the direction* of the definitive H2 result — the predictive hybrid beats the reactive hybrid in 7 of 8 config-matched pairs across the n=3 and n=5 replications, with a consistent negative p99 difference and fewer SLO violations. It does **not** reproduce the July statistical significance (p = 0.0958 vs July p = 0.0304): the effect size in the replication is smaller (Δ −21.8 ms / −16.5% vs July Δ −62.5 ms / −33.1%) because the reactive S3 baseline was faster in the replication runs (131.7 ms vs 188.5 ms), narrowing the gap. This is within-run variance across days and cluster states, not a mechanism failure: S4 still won 4/5 pairs, and pair 5 (S4 161.3 vs S3 147.2) shows the variance envelope. The recorded July statistics remain the thesis's inferential claim; the replication is presented as exactly what it is — an independent, configuration-matched re-run proving the experiment is real, repeatable, and directionally stable.
