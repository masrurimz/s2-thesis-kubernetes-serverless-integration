    ...
    def wait_healthy(self, timeout: Optional[int] = None) -> bool:
        ...
        while time.time() - start_time < timeout:
            ...
            if self.process and self.process.poll() is not None:
                raise RuntimeError(f"Process {self.config.name} died with code {self.process.returncode}")
        ...
        raise TimeoutError(f"Process {self.config.name} not healthy after {timeout}s: {last_error}")
