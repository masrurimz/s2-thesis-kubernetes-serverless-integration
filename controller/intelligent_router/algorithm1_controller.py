...
from monitoring_v2.slo_monitor import SLOMonitor, SLOStatus
...
class Algorithm1Controller:
    ...
    def __init__(self, slo_monitor: Optional[SLOMonitor] = None, config: Optional[Algorithm1Config] = None):
        """
        ...
        self.current_weights = {"k3s": self.config.default_k3s_weight, "knative": self.config.default_knative_weight}
        ...
        logger.info(
            "Algorithm1Controller initialized", weight_step=self.config.weight_step, cooldown=self.config.cooldown_sec
        )
    ...
    def make_decision(
        ...
        if slo_status.violation_duration_sec >= self.slo_monitor.config.violation_window_sec and can_adjust:
        ...
        if (
            slo_status.p99_latency_ms < healthy_threshold
            and can_adjust
    ...
    def _scale_out(self, current_time: int, slo_status: SLOStatus) -> RoutingDecision:
        ...
        new_knative = min(self.config.max_knative_weight, self.current_weights["knative"] + self.config.weight_step)
    ...
    def _apply_prediction(
        self, current_time: int, prediction: Dict, current_load: Optional[float]
    ...
    def _prewarm_knative(self) -> bool:
        ...
        try:
            logger.info("Pre-warming Knative service", url=self.config.knative_url, host=self.config.knative_host)
            ...
            if response.status_code == 200:
                logger.info("Knative pre-warm successful", cold_start_ms=cold_start_ms, status=response.status_code)
                return True
            else:
                logger.warning(
                    "Knative pre-warm returned non-200", status=response.status_code, cold_start_ms=cold_start_ms
                )
                return False
