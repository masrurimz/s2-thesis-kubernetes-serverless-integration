...
class FallbackHandler:
    ...
    def __init__(
        ...
        logger.info(
            "FallbackHandler initialized", safe_weights={"k3s": safe_k3s_weight, "knative": safe_knative_weight}
        )
    ...
    def _get_emergency_weights(self, stats: Dict) -> Dict[str, int]:
        ...
        logger.warning("Emergency fallback activated", reason=self.fallback_reason, stats=stats)
    ...
    def _get_high_load_weights(self, stats: Dict) -> Dict[str, int]:
        ...
        logger.info(
            "High load fallback activated", reason=self.fallback_reason, total_requests=stats.get("total_requests")
        )
    ...
    def _get_low_load_weights(self, stats: Dict) -> Dict[str, int]:
        ...
        logger.debug(
            "Low load fallback activated", reason=self.fallback_reason, total_requests=stats.get("total_requests")
        )
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            "safe_weights": {"k3s": self.safe_k3s_weight, "knative": self.safe_knative_weight},
            "emergency_weights": {"k3s": self.emergency_k3s_weight, "knative": self.emergency_knative_weight},
        }
    ...
    def get_recovery_weights(self, target_weights: Dict[str, int]) -> Dict[str, int]:
        ...
        try:
            ...
            current_weights = {"k3s": self.safe_k3s_weight, "knative": self.safe_knative_weight}
            ...
            logger.debug(
                "Recovery weights calculated", current=current_weights, target=target_weights, recovery=recovery_weights
            )
...
def main():
    ...
    emergency_stats = {"total_requests": 1000, "avg_response_time": 1500.0, "error_rate": 0.1}
